...
template <typename T>
requires StdOut<T> and std::equality_comparable<T>
class Graph {
    friend class Floyd<T>;
    friend class DijkstraPlus<T>;
...
public:
    ...
    bool if_vex_leaf(const T& v_name) {
        std::vector<int>&& all_adj_idx = FindAllAdjIndex(v_name);

        bool res = false;
        if (all_adj_idx.size() == 1) {
            // only one adj vex
            res = true;
        }
        return res;
...
public:
    int FindAdjIndex(const T& v_name) {
        ...
        for (int col_idx = 0; col_idx < size; ++col_idx) {
            bool if_adj = false;
            if (if_weighted) {
                if_adj = Mat[input_idx][col_idx] != 0  // == 0 => self
                    && Mat[input_idx][col_idx] != LIM; // == LIM => not connective
            } else {
                if_adj = Mat[input_idx][col_idx] != 0; // == 0 => not connective
            }
            if (if_adj) {
                index_ans = col_idx;
                break;
